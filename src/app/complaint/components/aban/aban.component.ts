...
export class AbanComponent implements AfterViewInit {
  ...
  @ViewChild('templateUCC') public templateUCC: TemplateRef<any>;
  ...
  Vehicletypelist = [];filteredVehicleTypes: Observable<any[]>;
  carvin_id: number;
  indLoading: boolean;
  LoaderImage: any;
  ...
  VINPlateMatchPermission: boolean;NoOfDaysPermission:boolean;
  ...
  IsAlley: boolean = false; IsStreet: boolean = false; IsPrivate: boolean = false;IsReadOnly:boolean=false;
  TowingId:number;RequestId:number;
      ...
      .subscribe(ButtonPermissionLists => {
        if (ButtonPermissionLists != null) {
          ...
          for (var i = 0; i < this.ButtonPermissionList.length; i++) {
            ...
            else if (this.ButtonPermissionList[i]['Control_Name'] == 'txtNoOfDays') {
              this.NoOfDaysPermission = Boolean(this.ButtonPermissionList[i]["view_hide"]);
            }
  ...
  private createForm() {
    this.AbanForm = new FormGroup({
      ...
      DaysNumFormControl: new FormControl(''),
  ...
  Save(obj) {
    ...
    if (this.AbanForm.valid) {
      ...
      if (obj.DaysNumFormControl) {
        noofdays = obj.DaysNumFormControl;
      }
      if (obj.VehicleTypeFormControl) {
        vehicleTypeId = obj.VehicleTypeFormControl.Vehicle_Type_Id;
      }
    ...
    this._dataService.post(Global.DLMS_API_URL + 'api/Aban/Search', searchobj)
      .subscribe(items => {
        ...
        if (items.length == 1) {
          ...
          this.LoadVehicleType(item.VehicleTypeId);
          /**/
...
if (item.IsPrivate) {
  (<FormControl>this.AbanForm.controls['RequestedByFormControl']).setValidators([Validators.required]);
        ...
        (this.AbanForm.controls['PhoneFormControl']).updateValueAndValidity();
          ...
          if(this.NoOfDaysPermission){
            (<FormControl>this.AbanForm.controls['DaysNumFormControl']).setValue(item.NoOfDays, {});
            (this.AbanForm.controls['DaysNumFormControl']).updateValueAndValidity();
          }
  ...
  checkValidInput(type) {
    setTimeout(() => {
      ...
      if (type == "violationreason") {
        ...
        }/*else if(obj.ViolationReasonFormControl.ABAN_REASON_ID==6){ commented as per Dave request on 07/07/2021
          if(this.RoleId==18 || this.RoleId==19){
            this.resetViolationReason();
            this.OpenUCCWarning(this.templateUCC);
          }
        }*/
      } else if (type == "state") {
        ...
        if (obj.VehicleTypeFormControl.Vehicle_Type_Id === undefined) {
  ...
  resetVehicleType(): void {
    setTimeout(() => {
      (this.AbanForm.controls['VehicleTypeFormControl']).setValue(null);
    }, 1);
  }
  filterVehicleTypes(val) {
    return val ? this.Vehicletypelist.filter(s => s.Description.toLowerCase().indexOf(val.toLowerCase()) === 0)
      : this.Vehicletypelist;
  }
  displayFnVehicleType(vt): string {
    return vt ? vt.Description : vt;
  }
  LoadVehicleType(VehicleTypeId): void {
    this._dataService.get(Global.DLMS_API_URL + 'api/Disposition/SelectVehicleType')
      .subscribe(list => {
        this.Vehicletypelist = list;
        this.filteredVehicleTypes = this.AbanForm.controls['VehicleTypeFormControl'].valueChanges
          .startWith(null)
          .map(vt => vt && typeof vt === 'object' ? vt.Description : vt)
          .map(name => this.filterVehicleTypes(name));
        if (VehicleTypeId > 0) {
          for (let vt of this.Vehicletypelist) {
            if (VehicleTypeId == vt.Vehicle_Type_Id) {
              (<FormControl>this.AbanForm.controls['VehicleTypeFormControl'])
                .setValue(vt, {});
...
*/
  ...
  /*OpenUCCWarning(template: TemplateRef<any>){
    this.modalUCCRef = this.modalService.show(template, Object.assign({}, this.config, { class: 'gray modal-md modal-dialog-centered' }));

  }
  closeUCCRef(){
    this.modalUCCRef.hide();
  }*/
  ...
  ChangeLocationType(event) {
    (<FormControl>this.AbanForm.controls['RequestedByFormControl']).setValidators(null);
    (this.AbanForm.controls['RequestedByFormControl']).updateValueAndValidity();
    (<FormControl>this.AbanForm.controls['RequestedByLastFormControl']).setValidators(null);
    (this.AbanForm.controls['RequestedByLastFormControl']).updateValueAndValidity();
    (<FormControl>this.AbanForm.controls['PhoneFormControl']).setValidators([Validators.maxLength(15), Validators.pattern(Global.PHONE_REGEX)]);
    (this.AbanForm.controls['PhoneFormControl']).updateValueAndValidity();
    if (event.checked == true) {
      this.IsAlley = false;
      this.IsStreet = false;
  ...
  RedirectTow(TowingId){    

    window.top.location.href = Global.PoliceURL + "Officer/TowEditNew.aspx?TID=" + TowingId+ "&FromList=SearchTow.aspx";
  }
  RedirectDispatch(RequestId){
    window.top.location.href = Global.PoliceURL + "Request/Request.aspx?Id=" + RequestId;
  }
